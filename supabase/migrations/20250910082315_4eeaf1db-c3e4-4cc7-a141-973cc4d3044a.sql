...
WITH contact_companies AS (
  SELECT id, company FROM public.contacts 
  WHERE company IN ('Swiss National Bank', 'State Secretariat for Economic Affairs', 'University of Zurich', 'UBS Group AG', 'BlackRock', 'Graduate Institute Geneva', 'European Central Bank', 'University of Lausanne', 'Federal Department of Finance')
)
...
CROSS JOIN LATERAL (
  SELECT UNNEST(CASE 
    WHEN cc.company = 'Swiss National Bank' THEN ARRAY['Central Banking', 'Monetary Policy', 'VIP']
    WHEN cc.company = 'State Secretariat for Economic Affairs' THEN ARRAY['Government', 'Trade Policy', 'Economics']
    WHEN cc.company = 'University of Zurich' THEN ARRAY['Academia', 'Research', 'Behavioral Economics']
    WHEN cc.company = 'UBS Group AG' THEN ARRAY['Banking', 'Financial Services', 'Leadership']
    WHEN cc.company = 'BlackRock' THEN ARRAY['Asset Management', 'Sustainable Finance', 'Former Central Banker']
    WHEN cc.company = 'Graduate Institute Geneva' THEN ARRAY['Academia', 'International Economics', 'Research']
    WHEN cc.company = 'European Central Bank' THEN ARRAY['Central Banking', 'EU Policy', 'International']
    WHEN cc.company = 'University of Lausanne' THEN ARRAY['Academia', 'Banking Research', 'Former SNB']
    WHEN cc.company = 'Federal Department of Finance' THEN ARRAY['Government', 'Financial Regulation', 'Minister']
    ELSE ARRAY['General']
  END) as tag
) t;
