-- Add interaction history for the new contacts with proper enum casting
INSERT INTO public.interactions (contact_id, type, date, description, channel, outcome, contacted_by)
SELECT 
  c.id,
  (ARRAY['meeting', 'email', 'phone_call', 'conference']::interaction_type[])[floor(random() * 4 + 1)],
...
-- Add upcoming opportunities for the contacts with proper enum casting
INSERT INTO public.opportunities (contact_id, type, title, date, location, description, registration_status)
SELECT 
  c.id,
  (ARRAY['conference', 'meeting', 'workshop', 'networking_event']::opportunity_type[])[floor(random() * 4 + 1)],
  CASE 
  ...
  (ARRAY['registered', 'pending', 'considering']::registration_status[])[floor(random() * 3 + 1)]
FROM public.contacts c
