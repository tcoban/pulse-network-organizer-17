...
SELECT 
  c.id,
  (ARRAY['meeting', 'email', 'call', 'conference']::interaction_type[])[floor(random() * 4 + 1)],
...
SELECT 
  c.id,
  (ARRAY['conference', 'meeting', 'workshop', 'event']::opportunity_type[])[floor(random() * 4 + 1)],
  CASE 
  ...
  (ARRAY['registered', 'pending', 'interested']::registration_status[])[floor(random() * 3 + 1)]
FROM public.contacts c
