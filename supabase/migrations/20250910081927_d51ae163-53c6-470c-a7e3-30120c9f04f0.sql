...
-- Insert enriched contacts with proper assignments using names instead of UUIDs
INSERT INTO public.contacts (
  ...
  -- Additional contacts assigned to other team members
  (
    gen_random_uuid(),
    'Dr. Martin Schlegel',
    'martin.schlegel@snb.ch',
    '+41 44 631 32 11',
    'Swiss National Bank',
    ...
    (SELECT id FROM auth.users WHERE email = 'tcoban@ethz.ch' LIMIT 1),
    'Central Bank',
    'Financial market operations, monetary policy implementation',
    'Operational research on monetary policy transmission',
    'Joint analysis of monetary policy tools effectiveness',
    'Interest rate transmission study, market operations research',
    'SNB Governing Board member responsible for monetary policy operations.',
    'Thomas Jordan',
    ARRAY['Thomas Jordan', 'Christine Lagarde', 'Philip Lane'],
    4,
    4,
  ...
  (
    ...
    'beatrice.weder@graduateinstitute.ch',
    '+41 22 908 59 00',
    'Graduate Institute Geneva',
    'Professor of International Economics',
    'Carmen Steiner',
    (SELECT id FROM auth.users WHERE email = 'tcoban@ethz.ch' LIMIT 1),
    'Academia',
    'International finance, sovereign debt, financial crises',
    'Collaboration on crisis management and international spillovers',
    'Research on crisis prevention and international financial architecture',
    'European debt crisis analysis, sovereign risk assessment',
    'Former German Council of Economic Experts member. Crisis expert.',
    'Kenneth Rogoff',
    ARRAY['Kenneth Rogoff', 'Carmen Reinhart', 'Olivier Blanchard'],
    4,
    5,
  ...
  (
    ...
    (SELECT id FROM auth.users WHERE email = 'tcoban@ethz.ch' LIMIT 1),
    'Financial Sector',
    'Digital banking transformation, sustainable finance',
    'Research on fintech impact and digital transformation',
    'Study on digital banking adoption and customer behavior',
    'Sustainable finance strategy, digital transformation in banking',
    'UBS CEO focusing on digital transformation and sustainability.',
    'Sergio Ermotti',
    ARRAY['Sergio Ermotti', 'Jamie Dimon', 'Ana Botín'],
    3,
    4,
    NOW() - INTERVAL '30 days',
  ...
  (
    ...
    'sabine.lautenschlaeger@consultant.eu',
    '+49 69 1344 8000',
    ...
    (SELECT id FROM auth.users WHERE email = 'tcoban@ethz.ch' LIMIT 1),
    'Financial Sector',
    'Banking supervision, regulatory expertise',
    'Academic collaboration on banking regulation effectiveness',
    'Research on supervisory practices and regulatory impact',
    'Banking supervision effectiveness study, regulatory arbitrage analysis',
    'Former ECB Executive Board member and BaFin President. Regulatory expert.',
    'Andrea Enria',
    ARRAY['Andrea Enria', 'Christine Lagarde', 'Pablo Hernández de Cos'],
    4,
    5,
  ...
  (
    ...
    (SELECT id FROM auth.users WHERE email = 'tcoban@ethz.ch' LIMIT 1),
    'Academia',
    'Monetary economics, central bank digital currencies',
    'CBDC research collaboration, monetary theory insights',
    'Joint research on CBDC design and monetary implications',
    'CBDC impact study, monetary sovereignty research',
    'Leading CBDC researcher. Consultant to central banks worldwide.',
    'Thomas Jordan',
    ARRAY['Thomas Jordan', 'Agustín Carstens', 'Benoit Coeuré'],
    5,
    4,
    NOW() - INTERVAL '6 days',
...
CROSS JOIN (
  SELECT UNNEST(CASE 
    ...
    WHEN c2.company = 'Independent Consultant' THEN ARRAY['Consulting', 'Former Regulator', 'Banking Supervision']
    WHEN c2.company = 'University of Bern' THEN ARRAY['Academia', 'CBDC Research', 'Monetary Economics']
    ELSE ARRAY['General']
  END) as tag
  FROM public.contacts c2 
  WHERE c2.id = c.id
) t(tag)
WHERE c.company IN ('Swiss National Bank', 'State Secretariat for Economic Affairs', 'University of Zurich', 'UBS Group AG', 'BlackRock', 'Graduate Institute Geneva', 'European Central Bank', 'University of Lausanne', 'Federal Department of Finance', 'Independent Consultant', 'University of Bern');
...
SELECT 
  ...
  c.assigned_to
FROM public.contacts c
WHERE c.company IN ('Swiss National Bank', 'State Secretariat for Economic Affairs', 'University of Zurich', 'UBS Group AG', 'BlackRock', 'European Central Bank', 'University of Bern');
...
SELECT 
  ...
  CASE 
    ...
    WHEN c.company = 'University of Bern' THEN 'CBDC Research Symposium'
  ...
  CASE 
    ...
    WHEN c.company = 'University of Bern' THEN 'Bern, Switzerland'
...
FROM public.contacts c
WHERE c.company IN ('Swiss National Bank', 'State Secretariat for Economic Affairs', 'University of Zurich', 'UBS Group AG', 'European Central Bank', 'Graduate Institute Geneva', 'University of Bern', 'University of Lausanne')
