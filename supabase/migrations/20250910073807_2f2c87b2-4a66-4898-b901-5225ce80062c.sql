...
-- Simplified version to avoid duplicate key issues

-- Insert 150+ contacts with proper cooperation and potential scores (1-5 range)
DO $$
DECLARE
    i INTEGER;
    contact_id UUID;
...
BEGIN
    ...
    FOR i IN 1..150 LOOP
        ...
        -- Add primary expertise tag (avoid duplicates)
        INSERT INTO public.contact_tags (contact_id, tag) 
        VALUES (contact_id, expertise_areas[(i % array_length(expertise_areas, 1)) + 1])
        ...
        -- Add some contact preferences for every 5th contact
        IF i % 5 = 0 THEN
